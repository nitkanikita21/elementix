...
sealed class InternalComponent<P : DefaultElementProps<E>, E : Element>(
    cx: Context, configBuilder: NativeComponentConfig<P>.() -> Unit = {}
) : NativeComponent<P, E>(cx, configBuilder) {
    open fun propsEffect() {
        props.initEffects(cx)
    }
}

class Div(
    cx: Context, configBuilder: NativeComponentConfig<DefaultHTMLElementProps<HTMLDivElement>>.() -> Unit = {}
) : InternalComponent<DefaultHTMLElementProps<HTMLDivElement>, HTMLDivElement>(cx, configBuilder) {

    override val element: HTMLDivElement = document.createElement("div") as HTMLDivElement
    override val config: NativeComponentConfig<DefaultHTMLElementProps<HTMLDivElement>> =
        NativeComponentConfig(this).apply(configBuilder)
    override val props = DefaultHTMLElementProps(element).apply(config.propsBuilder)
...
class P(
    cx: Context, configBuilder: NativeComponentConfig<DefaultHTMLElementProps<HTMLParagraphElement>>.() -> Unit = {}
) : InternalComponent<DefaultHTMLElementProps<HTMLParagraphElement>, HTMLParagraphElement>(cx, configBuilder) {

    override val element: HTMLParagraphElement = document.createElement("p") as HTMLParagraphElement
    override val config: NativeComponentConfig<DefaultHTMLElementProps<HTMLParagraphElement>> =
        NativeComponentConfig(this).apply(configBuilder)
    override val props = DefaultHTMLElementProps(element).apply(config.propsBuilder)
...
class Button(
    cx: Context, configBuilder: NativeComponentConfig<DefaultHTMLElementProps<HTMLButtonElement>>.() -> Unit = {}
) : InternalComponent<DefaultHTMLElementProps<HTMLButtonElement>, HTMLButtonElement>(cx, configBuilder) {

    override val element: HTMLButtonElement = document.createElement("button") as HTMLButtonElement
    override val config: NativeComponentConfig<DefaultHTMLElementProps<HTMLButtonElement>> =
        NativeComponentConfig(this).apply(configBuilder)
    override val props = DefaultHTMLElementProps(element).apply(config.propsBuilder)
...
/*
sealed class InternalHTMLComponent<P : DefaultHTMLElementProps<E>, E : HTMLElement>(
    cx: Context, configBuilder: ComponentConfig<P, E>.() -> Unit = {}
) : Component<P, E>(cx, configBuilder) {
